...
static void print_time(const git_time *intime, char *time);
...
static void print_time(const git_time *intime, char *time)
{
	...
	sprintf(time, "%s %c%02d%02d", out, sign, hours, minutes);
...
static void print_commit(git_commit *commit, struct log_options *opts)
{
	...
	if (opts->show_log_size) {
		...
		data.log_size = (int)strlen(git_commit_message(commit));
	} else {
		data.log_size = 0;
	}
	...
	if ((count = (int)git_commit_parentcount(commit)) > 1) {
		...
		for (i = 0; i < count; ++i) {
			git_oid_tostr(buf, 8, git_commit_parent_id(commit, i));
			strcat(str, buf);
			str = (char *) realloc(str, sizeof(char) * (strlen(str) + 8));
		}
		char *new_str[strlen(str) + 1];
		strcpy(new_str, str);
		free(str);
		data.parent_ids = new_str;
	}
	...
	data.commit_message = scan;


	char time[50] = "";
	print_time(&sig->when, time);

	EM_ASM({
		if (!Reflect.has(Module,"onLogData")) return;
		Module.onLogData({
			logSize: $0,
			commitId: UTF8ToString($1),
			partentIds: UTF8ToString($2),
			commitMessage: UTF8ToString($3),
			author: {
				name: UTF8ToString($4),
				email: UTF8ToString($5)
			},
			commitTime: UTF8ToString($6),
		});
	},
		data.log_size,
		data.commit_id,
